{block name='clerk_article'}
    {if isset($s360_clerk_article) && !empty($s360_clerk_article.template)}
        {$filter = ""}
        {$templates = explode(',', $s360_clerk_article.template)}
        {$excludeDuplicates = $s360_clerk_article.exclude}

        {foreach from=$templates item=$template key=$key}
            {block name='clerk_article_snippets'}
            ...
            {block name='clerk_article_api'}
                <span class="clerk {if $excludeDuplicates}clerk_{$key|escape:"htmlall"}{/if}"
                    ...
                    data-products="[{$Artikel->kArtikel}]"
