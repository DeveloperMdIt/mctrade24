...
{function containerContent}
    {if $instance->getProperty('background-flag') === 'video' && !empty($instance->getProperty('video-src'))}
        <video autoplay loop playsinline muted poster="{$videoPosterUrl}"
...
<div class="adm-portlet">
    {if $instance->getProperty('full-width')}
        {$class = "{$class} container-fluid container--{$admPro->container_size()}"}
    ...
    {else}
        {container style=$style class=$class data=$data}
            {call containerContent}
        {/container}
    {/if}
