...
class CheckoutController extends RegistrationController
{
    ...
    public function getResponse(ServerRequestInterface $request, array $args, JTLSmarty $smarty): ResponseInterface
    {
        ...
        if (isset($_SESSION['Kunde']) && $_SESSION['Kunde']) {
            if (!isset($_SESSION['Lieferadresse'])) {
                ...
                $form->pruefeLieferdaten(['kLieferadresse' => \max($shippingID, 0)]);
    ...
    public function checkGuestOrderProcess(array $post): int
    {
        ...
        $missingInput       = $form->getMissingInput($post, $this->customerGroupID ?: null, $checkBox);
        ...
        if (isset($post['shipping_address'])) {
            if ((int)$post['shipping_address'] === 0) {
                ...
                Tax::setTaxRates();
            } elseif ((int)($post['kLieferadresse'] ?? 0) > 0) {
                $form->pruefeLieferdaten($post);
    ...
    public function checkPaymentMethod(int $paymentMethodID): int
    {
        ...
        if ($paymentMethod === null) {
            ...
            if ($paymentMethod === null || $paymentMethod->cModulId !== 'za_null_jtl') {
                return 0;
            }
        }
            ...
            switch ($paymentMethod->cModulId) {
                ...
                case 'za_lastschrift_jtl':
                    ...
                    if (\count($fehlendeAngaben) === 0) {
                        $info->cBankName = Text::htmlentities(\stripslashes(Request::pString('bankname')), \ENT_QUOTES);
                        $info->cKontoNr  = Text::htmlentities(\stripslashes(Request::pString('kontonr')), \ENT_QUOTES);
                        $info->cBLZ      = Text::htmlentities(\stripslashes(Request::pString('blz')), \ENT_QUOTES);
                        $info->cIBAN     = Text::htmlentities(\stripslashes(Request::pString('iban')), \ENT_QUOTES);
                        $info->cBIC      = Text::htmlentities(\stripslashes(Request::pString('bic')), \ENT_QUOTES);
                        $info->cInhaber  = Text::htmlentities(\stripslashes(Request::pString('inhaber')), \ENT_QUOTES);
    ...
    {
        if (isset($_SESSION['Zahlungsart'])) {
            $_SESSION['AktiveZahlungsart'] = (int)$_SESSION['Zahlungsart']->kZahlungsart;
            ...
            if (
                ...
                $_SESSION['AktiveZahlungsart'] = (int)$shippingMethods[0]->kZahlungsart;
            }
        } else {
            $_SESSION['AktiveZahlungsart'] = (int)$shippingMethods[0]->kZahlungsart;
        }
    ...
    public function getPaymentMethods(int $shippingMethodID, int $customerGroupID): array
    {
        ...
        foreach ($methods as $method) {
            ...
            $method->fAufpreis              = (float)$method->fAufpreis;
            ...
            // evtl. Versandkupon anwenden / nur Nachnahme fällt weg
            if (
                isset($_SESSION['VersandKupon']->cZusatzgebuehren)
                && $_SESSION['VersandKupon']->cZusatzgebuehren === 'Y'
                && $method->fAufpreis > 0
     ...
     * @since 5.2.0
     */
    public function getPluginPaymentMethod(string $moduleID): false|PluginInterface
    {
     ...
     */
    public function getCustomerAccountData(?int $customerID): false|stdClass
    {
        if ($customerID === null) {
            return false;
        }
     ...
     */
    public function checkAdditionalPayment(Zahlungsart|stdClass $paymentMethod): array
    {
    ...
    public function checkStepShippingCosts(): string
    {
        ...
        $dependent = $this->getShippingService()->getCustomShippingCostsByCart(
        ...
        foreach ($dependent as $item) {
            $langISOCode = Shop::Lang()->getLanguageCode();
            if (empty($item->nameLocalized[$langISOCode])) {
                continue;
            }

            $this->cart->erstelleSpezialPos(
                $item->nameLocalized[$langISOCode],
                1,
                $item->netPrice,
                $item->taxClassID,
    ...
    public function checkStepBillingAddress(): string
    {
        ...
        if (
            !empty($this->customer->cOrt)
            && empty($this->getShippingService()->getPossibleShippingCountries(
                [(string)$this->customer->cLand],
                $this->customerGroupID,
                $this->cart->PositionenArr,
            ))
        ) {
            ...
            if (
                !isset($_SESSION['Lieferadresse'])
                || empty($this->getShippingService()->getPossibleShippingCountries(
                    [(string)Frontend::getDeliveryAddress()->cLand],
                    $this->customerGroupID,
                    $this->cart->PositionenArr,
                ))
            ) {
    ...
    public function checkStepDeliveryAddress(array $get): string
    {
        ...
        if (!empty($_SESSION['Lieferadresse'])) {
            ...
            if ($Lieferadresse instanceof Lieferadresse && $Lieferadresse->isValidForCheckout() === false) {
                $Lieferadresse = Lieferadresse::createFromShippingAddress();
            }
            if (
    ...
    public function checkStepAccountSelection(): void
    {
        ...
        $this->smarty->assign('untertitel', Texts::orderCartContainsItems($this->cart))
    ...
    public function getStepGuestCheckout(): void
    {
        ...
        $this->smarty->assign('untertitel', Shop::Lang()->get('fillUnregForm', 'checkout'))
            ...
            ->assign('LieferLaender', $this->getShippingService()->getPossibleShippingCountries(
                [],
                $this->customerGroupID,
                $this->cart->PositionenArr,
            ))
     ...
     */
    public function getStepDeliveryAddress(): mixed
    {
        global $Lieferadresse;
        ...
        $this->smarty
            ...
            ->assign(
                'LieferLaender',
                $this->getShippingService()->getPossibleShippingCountries(
                    [],
                    $this->customerGroupID,
                    $this->cart->PositionenArr,
                )
            ...
            ->assign('kLieferadresse', $_SESSION['Bestellung']->kLieferadresse ?? -1)
            ->assign('shippingAddressPresetID', $_SESSION['shippingAddressPresetID'] ?? null);
        if (($_SESSION['Bestellung']->kLieferadresse ?? 0) === -1) {
    ...
    public function getStepShipping(): void
    {
        ...
        $shippingMethods = \array_map(
            ...
            $this->getShippingService()->getPossibleShippingMethods(
                $this->customer,
                new CustomerGroup($this->customerGroupID),
                $deliveryCountry,
                Frontend::getCurrency(),
        ...
        if (
            ...
            || (\count($shippingMethods) === 1 && GeneralObject::hasCount($packagings))
        ) {
        ...
        } elseif (\count($shippingMethods) === 0) {
            Shop::Container()->getLogService()->error(
                ...
                [
                    'cny'    => $deliveryCountry,
                    'zip'    => $poCode,
                    'sclass' => \implode('-', $this->getShippingService()->getShippingClasses()),
    ...
    public function getStepPayment(): void
    {
        ...
        if (GeneralObject::hasCount($shippingMethods)) {
            $shippingMethodID = $this->getActiveShippingMethod($shippingMethods);
            $paymentMethods   = $this->getPaymentMethods($shippingMethodID, $this->customerGroupID);
            if (\count($paymentMethods) === 0) {
                Shop::Container()->getLogService()->error(
                    'Es konnte keine Zahlungsart für folgende Daten gefunden werden: Versandart: {name},'
                    . ' Kundengruppe: {cgid}',
                    ['name' => $shippingMethodID, 'cgid' => $this->customerGroupID]
                );
                $paymentMethod  = null;
                $paymentMethods = [];
            } else {
            ...
            if (!isset($_SESSION['Versandart']) && !empty($shippingMethodID)) {
                // dieser Workaround verhindert die Anzeige der Standardzahlungsarten wenn ein Zahlungsplugin aktiv ist
                $_SESSION['Versandart'] = (object)['kVersandart' => $shippingMethodID];
            }
            ...
            $this->smarty->assign('Zahlungsarten', $selectablePayments)
                ...
                ->assign('Verpackungsarten', $packagings)
                ->assign('AktiveVersandart', $shippingMethodID)
    ...
    {
        $paymentID     = (int)($post['Zahlungsart'] ?? $_SESSION['Zahlungsart']->kZahlungsart);
    ...
    public function getPaymentSurchageDiscount(?stdClass $paymentMethod): void
    {
        ...
        $this->cart->erstelleSpezialPos(
            ...
            1,
            $surcharge,
            $this->getShippingService()->getTaxRateIDs(
                '',
                $this->cart->PositionenArr,
                $_SESSION['Lieferadresse']->cLand ?? '',
            )[0]->taxRateID ?? 0,
    ...
    public function getStepConfirmation(array $get): void
    {
        ...
        // falls zahlungsart extern und konfiguriert, dass Bestellung für Kaufabwicklung notwendig,
        // füllte tzahlungsession
        $this->smarty->assign('Kunde', $this->customer)
    ...
    public function shippingMethodIsValid(int $shippingMethodID, ?array $formValues = null): bool
    {
            ...
            foreach ($packagingIDs as $packagingID) {
                ...
                if ($packaging->kSteuerklasse < 1) {
                    if ($defaultTaxClassID === 0) {
                        $defaultTaxClassID = $this->getShippingService()->getTaxRateIDs(
                            '',
                            $this->cart->PositionenArr,
                            $_SESSION['Lieferadresse']->cLand ?? '',
                        )[0]->taxRateID ?? 0;
        ...
        $shippingNetCosts = $this->getShippingService()->calcCostForShippingMethod(
            $shippingMethod,
            $countryCode,
            $this->customer,
            $this->cart->PositionenArr,
        );
        if ($shippingNetCosts === -1.0) {
            return false;
        }
        ...
        $shippingNetPrice = $shippingMethod->finalNetCost - ($shippingMethod->shippingSurcharge->netSurcharge ?? 0.00);
        $this->cart->erstelleSpezialPos(
            $specialItem->cName,
            1,
            $shippingNetPrice,
        ...
        if (
            isset($shippingMethod->shippingSurcharge->netSurcharge)
            && $this->getShippingService()->isZero($shippingMethod->shippingSurcharge->netSurcharge) === false
        ) {
            $specialItem->cName = [];
            foreach (Frontend::getLanguages() as $lang) {
                $loc                             = $this->db->select(
                    ...
                    'kVersandzuschlag',
                    $shippingMethod->shippingSurcharge->surchargeID,
                    'cISOSprache',
                    $lang->cISO,
                    null,
                    null,
            ...
            $this->cart->erstelleSpezialPos(
                $specialItem->cName,
                1,
                $shippingMethod->shippingSurcharge->netSurcharge,
                $this->getShippingService()->getTaxRateIDs(
                    '',
                    $this->cart->PositionenArr,
                    $countryCode,
        ...
        $_SESSION['Versandart']       = $shippingMethod->toLegacyObject();
        $_SESSION['AktiveVersandart'] = $shippingMethod->id;
    ...
    private function getPackagings(): array
    {
        $packagings = \array_map(
            static function (object $packaging): stdClass {
                $result = new stdClass();
                foreach (\get_object_vars($packaging) as $key => $value) {
                    $result->$key = $value;
                }

                return $result;
            },
            $this->getShippingService()->getPossiblePackagings(
                $this->customerGroupID,
                Frontend::getCurrency(),
                $this->cart->PositionenArr,
            )
        ...
        if (
            empty($packagings)
            || $this->cart->posTypEnthalten(\C_WARENKORBPOS_TYP_VERPACKUNG)
        ) {
            return $packagings;
        }
        foreach (
            \array_filter(
                $this->cart->PositionenArr,
                static fn($item) => $item->nPosTyp === \C_WARENKORBPOS_TYP_VERPACKUNG
            ) as $packagingItemInCart
        ) {
            foreach ($packagings as $packaging) {
                $packaging->bWarenkorbAktiv =
                    $packaging->cName === $packagingItemInCart->getName($packaging->cISOSprache);
            }
        }
