...
class ProductController extends AbstractController
{
    ...
    public function init(): bool
    {
        ...
        parent::init();
        if ((int)$this->config['global']['global_sichtbarkeit'] === 3 && Frontend::getCustomer()->getID() === 0) {
            return false;
        }
    ...
    public function getResponse(ServerRequestInterface $request, array $args, JTLSmarty $smarty): ResponseInterface
    {
        ...
        if (Frontend::getCustomer()->getID() > 0) {
            $rated = ProductHelper::getRatedByCurrentCustomer(
                (int)$this->currentProduct->kArtikel,
                (int)$this->currentProduct->kVaterArtikel
            );
        }
        ...
        if (
            ...
            && empty($_SESSION['lastVisitedProductListURL'])
        ) {
            $category = new Kategorie($AktuellerArtikel->gibKategorie($this->customerGroupID ?: null));
            Frontend::set('lastVisitedProductListURL', $category->getURL($this->languageID ?: null));
        }
        $this->preRender();
    ...
    {
        if (
            $this->currentProduct?->kArtikel === null
