                ...
                {block name='snippets-linkgroup-recursive-list'}
                    {foreach $links as $li}
                        ...
                        {$active = $li->getIsActive()
                            ...
                            || ($li->getID() == $activeId)}
                        {if $hasItems}
                            <li class="link-group-item recursive-item nav-item {if $hasItems}dropdown{/if}{if $active} active{/if}">
                                <div class="nav-link d-flex position-relative {if $i !== 0}snippets-categories-nav-link-child{/if} dropdown-toggle pr-0">
                                    {block name='snippets-linkgroup-recursive-link'}
                                        <a class="link-group-item__link recursive-item__link position-relative" target="_self" href="{$li->getURL()}">
                                            {$li->getName()}
                                        </a>
                                    {/block}
                                    {block name='snippets-linkgroup-recursive-toggle-button'}
                                        <button
                                            class="link-group-item__toggle border-0 p-0 bg-transparent ml-auto recursive-item__nav-toggle" 
