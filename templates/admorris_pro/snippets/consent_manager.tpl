...
<div class="consent-manager-wrapper">
    ...
    {if isset($oSpezialseiten_arr[$smarty.const.LINKTYP_DATENSCHUTZ])}
        ...
        {$privacyName = $oSpezialseiten_arr[$smarty.const.LINKTYP_DATENSCHUTZ]->getName()}
        {$imprintUrl = $oSpezialseiten_arr[$smarty.const.LINKTYP_IMPRESSUM]->getURL()}
        {$imprintName = $oSpezialseiten_arr[$smarty.const.LINKTYP_IMPRESSUM]->getName()}
    {/if}
    		...
    		<div id="consent-banner">
    			{block name='snippets-consent-manager-banner-icon'}
    				<div class="consent-banner-icon icon-content--default">
                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><use href="#fingerprint"/></svg>
    				</div>
    			{/block}
    		...
    		<div id="consent-settings" class="consent-modal" role="dialog" aria-modal="true" aria-labelledby="consent-settings-title">
                <div class="consent-modal-content" role="document">
    				{block name='snippets-consent-manager-settings-close'}
    					<button type="button" class="consent-modal-close" data-toggle="consent-close" aria-label="{lang key='close' section='consent'}">
    				...
    				{block name='snippets-consent-manager-settings-title'}
    					<h2 id="consent-settings-title" class="consent-display-1">{lang key='cookieSettings' section='consent'}</h2>
    				{/block}
    					...
    					{foreach $consentItems as $item}
    						...
    						<div class="consent-switch">
    							{block name='snippets-consent-manager-settings-items-checkbox'}
    								<input type="checkbox" class="consent-input" id="consent-{$id}" name="consent-{$id}" {if $item->getItemID() === 'necessary'}
                                        disabled checked
                                                {else}
    							...
    							{block name='snippets-consent-manager-settings-items-more-content'}
    								<div class="consent-help consent-more-description consent-hidden" id="consent-{$id}-description">
    									...
    									<a href="{$item->getPrivacyPolicy()}" target="_blank" rel="noopener">
                                            {$item->getPrivacyPolicy()|truncate:100:"[...]":true}
                                        </a>
    								</div>
    							{/block}
    						</div>
    			...
    			{block name='snippets-consent-manager-confirm-content'}
    				<div class="consent-modal-content">
    					{block name='snippets-consent-manager-confirm-close'}
    						<button type="button" class="consent-modal-close" data-toggle="consent-close icon-content--default" aria-label="{lang key='close' section='consent'}">
    							<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path fill="currentColor" d="M207.6 256l107.72-107.72c6.23-6.23 6.23-16.34 0-22.58l-25.03-25.03c-6.23-6.23-16.34-6.23-22.58 0L160 208.4 52.28 100.68c-6.23-6.23-16.34-6.23-22.58 0L4.68 125.7c-6.23 6.23-6.23 16.34 0 22.58L112.4 256 4.68 363.72c-6.23 6.23-6.23 16.34 0 22.58l25.03 25.03c6.23 6.23 16.34 6.23 22.58 0L160 303.6l107.72 107.72c6.23 6.23 16.34 6.23 22.58 0l25.03-25.03c6.23-6.23 6.23-16.34 0-22.58L207.6 256z"/></svg>
    						</button>
    					{/block}
