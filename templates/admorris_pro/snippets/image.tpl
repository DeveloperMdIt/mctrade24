{*custom*}
{* Copied & modified from NOVA Template *}
...
{block name='snippets-image'}
    ...
    {block name='snippets-image-variables'}
        ...
        {$progressiveLoading = (!empty($progressiveLoading)) ? $item->getImage(\JTL\Media\Image::SIZE_XS) : false}
    {/block}
    ...
    {block name='snippets-image-main'}
        ...
        {/if}*}
            {block name='snippets-image-main-image'}
                {image
                    fluid=$fluid lazy=$lazy webp=$webp center=$center
            ...
            {/block}
            {* {if !empty($item->getImage(\JTL\Media\Image::SIZE_MD))}
                <meta itemprop="image" content="{$ShopURL}/{$item->getImage(\JTL\Media\Image::SIZE_MD)}">
            {/if} *}
        {* {if $square}
            </div>
        </div>
