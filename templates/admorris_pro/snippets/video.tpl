{if !empty($title)}
    {* admorris custom INFO: using aria-label instead of <label>. <label> would need a for attribute to work, but we have none. *}
    {* <label>{$title}</label> *}
...
<div class="{if !empty($class)}{$class}{/if}
    ...
    {elseif $video->getType() === \JTL\Media\Video::TYPE_FILE}
        ...
        <video class="product-detail-video mw-100" preload="metadata"
            ...
            {if !$video->isAllowFullscreen()}controlslist="nofullscreen"{/if}{$ariaLabel|default:''}>
