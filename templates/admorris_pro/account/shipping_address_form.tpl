{* admorris pro - custom: using lg breakpoint *}
{block name='account-shipping-address-form'}
    {block name='account-shipping-address-form-form'}
        ...
        {/if}
        {row}
            {col cols=12 lg={($isModal) ? 12 : 6} class='shipping-address-form-wrapper'}
            ...
            {/col}
            {if !$isModal}
                {col cols=12 lg=6 class='shipping-addresses-wrapper'}
                                ...
                                {foreach $Lieferadressen as $address}
                                    <tr>
                                        <td>
                                            ...
                                            {button variant="link" class="btn-show-more" aria=['expanded' => 'false']
                                        ...
                                        <td class="text-right">
                                            {buttongroup}
                                                {if $Einstellungen.kaufabwicklung.bestellvorgang_kaufabwicklungsmethode == 'N' && $address->nIstStandardLieferadresse !== 1}
                                                    <button type="button" class="btn btn-outline-secondary btn-sm" data-toggle="tooltip" data-placement="top" title="{lang key='useAsDefaultShippingAddress' section='account data'}" onclick="location.href='{get_static_route id='jtl.php' params=['editLieferadresse' => 1, 'setAddressAsDefault' => {$address->kLieferadresse}, 'token' => {$smarty.session.jtl_token}]}'">
                                                        {lang key='setAsStandard' section='account data'}
                                                    </button>
                                                {/if}

                                                <button type="button" class="btn btn-secondary btn-sm" data-toggle="tooltip" data-placement="top" title="{lang key='editAddress' section='account data'}" aria-label="{lang key='editAddress' section='account data'}" onclick="location.href='{get_static_route id='jtl.php' params=['editLieferadresse' => 1, 'editAddress' => {$address->kLieferadresse}]}'">
                                                    <span class="fas fa-pencil-alt"></span>
                                                </button>

                                                <button type="button" class="btn btn-danger btn-sm delete-popup-modal" data-lieferadresse="{$address->kLieferadresse}" data-toggle="tooltip" data-placement="top" title="{lang key='deleteAddress' section='account data'}" aria-label="{lang key='deleteAddress' section='account data'}">
    ...
    {block name='account-shipping-address-form-script'}
        ...
        $(document).ready(function () {
            ...
            $(tableID + ' tbody').on('click', '.delete-popup-modal',function(){
                ...
                let jtl_token     = $('.jtl_token:first').val();
                ...
                eModal.confirm(options).then(
                    function() {
                        window.location = "{get_static_route id='jtl.php'}?editLieferadresse=1&deleteAddress="+lieferadresse+"&token="+jtl_token
                    }
                );
            });
        });
