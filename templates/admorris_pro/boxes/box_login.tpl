            ...
            {else}
                {block name='boxes-box-login-actions'}
                    ...
                    {link href="{get_static_route id='jtl.php'}" class="btn btn-outline-primary btn-block btn-sm btn-account"}
                        {lang key='myAccount'}
                    {/link}
                    {link href="{get_static_route id='jtl.php'}?bestellungen=1" class="btn btn-outline-primary btn-block btn-sm btn-account"}
                        {lang key='myOrders'}
                    {/link}
                    {link href="{get_static_route id='jtl.php'}?editRechnungsadresse=1" class="btn btn-outline-primary btn-block btn-sm btn-account"}
                        {lang key='myPersonalData'}
                    {/link}
                    {link href="{get_static_route id='jtl.php'}?editLieferadresse=1" class="btn btn-outline-primary btn-block btn-sm btn-account"}
                        {lang key='myShippingAddresses'}
                    {/link}
                    {if $Einstellungen.global.global_wunschliste_anzeigen === 'Y'}
                        {link href="{get_static_route id='wunschliste.php'}" class="btn btn-outline-primary btn-block btn-sm btn-account"}
