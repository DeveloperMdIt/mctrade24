                    ...
                    {nav vertical=true class="box-nav-item"}
                        {foreach $oBox->getItems() as $newsMonth}
                            {block name='boxes-box-news-month-news-link'}
                                {navitem href=$newsMonth->cURLFull title="{$newsMonth->cName} - {$newsMonth->nAnzahl} {lang key='news'}" router-class="box-link-wrapper"}
                                    <i class="far fa-newspaper snippets-filter-item-icon-right mr-2"></i>
                                    {$newsMonth->cName}
