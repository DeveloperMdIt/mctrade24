{block name='boxes-box-categories'}
    <div class="box box-normal box-categories box-recursive word-break" id="sidebox-categories-{$oBox->getID()}">
        {block name='boxes-box-categories-content'}
            {block name='boxes-box-categories-toggle-title'}
                {link id="crd-hdr-{$oBox->getID()}"
                    ...
                    <span class="text-truncate">{if !empty($oBox->getTitle())}{$oBox->getTitle()}{else}{lang key='categories'}{/if}</span>
                {/link}
            {/block}
            ...
            {block name='boxes-box-categories-collapse'}
                {collapse
                    class="box-main-collapse d-md-block"
                    visible=false
