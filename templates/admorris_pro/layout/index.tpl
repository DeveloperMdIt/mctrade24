    ...
    {else}
        ...
        {block name='layout-index-content'}
            ...
            {if !empty($Link->getTitle())}
                ...
                <h1 class="page-heading">{$Link->getTitle()}</h1>
            {elseif isset($bAjaxRequest) && $bAjaxRequest}
                {opcMountPoint id='opc_before_heading'}
                <h1>{if !empty($Link->getMetaTitle())}{$Link->getMetaTitle()}{else}{$Link->getName()}{/if}</h1>
            {/if}

            {if $Link->getName() === "Startseite"}
                {lang key="startSubheading" section="custom" assign=startSubheading}
                {if !empty($startSubheading)}
                    <div class="subheading">{$startSubheading}</div>
                {/if}
            {/if}
            {/block}
            ...
            {block name='layout-index-link-content'}
                {if !empty($Link->getContent())}
                    ...
                    <div
                        class="page-content{if $Link->getName() === "Startseite"} page-content--center{/if}">
                        {$Link->getContent()}
                    </div>
                {/if}
            {/block}
            ...
            {if $Link->getLinkType() === $smarty.const.LINKTYP_AGB}
                ...
                <div id="tos" {*  class="card card-body well-sm" *}>
            ...
            {elseif $Link->getLinkType() === $smarty.const.LINKTYP_WRB}
                ...
                <div id="revocation-instruction" {*  class="card card-body well-sm" *}>
            ...
            {elseif $Link->getLinkType() === $smarty.const.LINKTYP_WRB_FORMULAR}
                ...
                <div id="revocation-form" {*  class="card card-body well-sm" *}>
            ...
            {elseif $Link->getLinkType() === $smarty.const.LINKTYP_DATENSCHUTZ}
                ...
                <div id="data-privacy" {*  class="card card-body well-sm" *}>
