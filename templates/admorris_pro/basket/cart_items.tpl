...
{block name='basket-cart-items'}
    ...
    {block name='basket-cart-items-order-items'}
        ...
        {foreach JTL\Session\Frontend::getCart()->PositionenArr as $oPosition}
            {$isExtraProduct = $oPosition->cResponsibility === 'admorris_pro_extra_product'}
            {$isNonBuyable = !empty($oPosition->Artikel->FunktionsAttribute["extra_product_not_buyable"])}
            {$hideInfo = !empty($oPosition->Artikel->FunktionsAttribute['extra_product_hide_info'])}
            {if !$oPosition->istKonfigKind()}
                {$posName=$oPosition->cName|transByISO|escape:'quotes'}
                {row class="cart-items-body type-{$oPosition->nPosTyp}"}
                    ...
                    {/block}
                    {col}
                        {row class="align-items-baseline row-gap row-gap-3"}
                            {block name='basket-cart-items-items-main-content'}
                                ...
                                {block name='basket-cart-items-quantity-outer'}
                                    {col cols=$cols xl=4 md=$mdLeft class="cart-items-quantity text-left text-xl-center"}
                                        {* {if $isExtraProduct} *}
                                        ...
                                        {if $oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_ARTIKEL}
                                            ...
                                            {else}
                                                ...
                                                {quantityInput name="anzahl[{$oPosition@index}]" article=$oPosition->Artikel wrapperClass="quantity-input quantity-input--cart" value=$oPosition->nAnzahl}
                                            {/if}
                            ...
                            {block name='basket-cart-items-order-items-price-net'}
                                {col cols=$cols xl=2 md=$mdRight class="cart-items-price price-col"}
                                    ...
                                    <span class="price_overall text-accent">
                                        {if $oPosition->istKonfigVater() && !isExtraProduct}
                            ...
                            {col cols=$cols xl=$cols+1 class="ml-auto"}
                                {block name='basket-cart-items-product-data'}
                                    {if !$hideInfo && ($oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_ARTIKEL
                                    || $oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_GRATISGESCHENK)}
                                ...
                                {block name='basket-cart-items-product-partlist-items-outer'}
                                    {if $Einstellungen.kaufabwicklung.bestellvorgang_partlist === 'Y' && !empty($oPosition->Artikel->kStueckliste) && !empty($oPosition->Artikel->oStueckliste_arr)}
                                        <ul class="partlist-items text-muted small mt-3 mb-0">
                            ...
                            {block name='basket-cart-items-cart-submit'}
                                {if !$isExtraProduct && ($oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_ARTIKEL
                                || $oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_GRATISGESCHENK
                                )}
                                    {col cols=$cols class='cart-items-delete' data=['toggle'=>'product-actions']}
                                        ...
                                        && $oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_ARTIKEL}
                                            {block name='basket-cart-items-cart-submit-include-wishlist-button'}
                                                {include file='snippets/wishlist_button.tpl' Artikel=$oPosition->Artikel buttonAndText=false variant='link'}
                                            {/block}
                                        {/if}
                                        {button type="submit"
                                            variant="link"
                                            ...
                                            {$admIcon->renderIcon('trash', 'icon-content icon-content--default icon-content--center')}
                                        {/button}
                                    {/col}
                                {/if}
                            {/block}
