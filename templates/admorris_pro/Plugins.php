 ...
 * Different from Nova:
 *
 * getCategoryArray: different from Nova .
 * Still using the Evo function to make offcanvas category loading work
 *
 * getBoxesByPosition: customized admorris - remove empty boxes
 */
...
class Plugins
{
     ...
     */
    public function __construct(private DbInterface $db, private JTLCacheInterface $cache, private TemplateUtils $utils)
    {
    ...
    /**
     * Customized to remove empty boxes
     *
     * @param array                    $params
     * @param Smarty_Internal_Template $smarty
     * @return array|void
    ...
    public function getBoxesByPosition(array $params, Smarty_Internal_Template $smarty)
    {
        ...
        // custom admorris - remove empty boxes
     ...
     * Category Array
     *
     * do not update to Nova version, because offcanvas category loading does not work
     *
    ...
    public function getCategoryArray(array $params, Smarty_Internal_Template  $smarty)
    {
        ...
        /** newer nova code - not working with offcanvas categories */
        // $id = isset($params['categoryId']) ? (int)$params['categoryId'] : 0;
        // if ($id === 0) {
        //     $categories = Category::getInstance();
        //     $list       = $categories->combinedGetAll();
    ...
    public function hasCheckBoxForLocation(array $params, Smarty_Internal_Template $smarty): void
    {
        ...
        foreach ($checkboxes as $key => $checkbox) {
            if (
                defined('JTL\\CheckBox::CHECKBOX_DOWNLOAD_ORDER_COMPLETE')
    ...
    public function getImageSize(?string $image): ?stdClass
    {
        ...
        } elseif (\file_exists($this->utils->getWebpImage($path))) {
            [$width, $height, $type] = \getimagesize($this->utils->getWebpImage($path));
        } else {
