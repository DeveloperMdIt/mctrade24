{* copied from NOVA template *}
{block name='productlist-productlist-actions'}
    {form action="#" method="post" class="productlist-actions product-actions" data=["toggle" => "product-actions"]}
        {block name='productlist-productlist-actions-buttons'}
            {if !($Artikel->nIstVater && $Artikel->kVaterArtikel === 0)}
                {if $Einstellungen.vergleichsliste.vergleichsliste_anzeigen === 'Y' && $Einstellungen.artikeluebersicht.artikeluebersicht_vergleichsliste_anzeigen === 'Y'}
