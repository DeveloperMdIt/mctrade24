{* custom *}
{* modified for extraproduct salesbooster *}
...
{block name='checkout-inc-order-items'}
    ...
    {$quantityColModifier = ''}
    {if !isset($Einstellungen.template.theme.qty_modify_dropdown) || $Einstellungen.template.theme.qty_modify_dropdown === 'N'}
        {$quantityColModifier = ' order-items__quantity-col--wide'}
    {/if}
                        ...
                        {row class='checkout-items-info'}
                            {block name='checkout-inc-order-items-items-main-content'}
                                {col cols=$cols lg=$itemInfoCols class="checkout-items-item-main"}
                                    ...
                                    || $oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_GRATISGESCHENK}
                                        {block name='checkout-inc-order-items-product-data-link'}
                                            {if empty($oPosition->Artikel->FunktionsAttribute["extra_product_not_buyable"])}
                                            <p class="h4">{link href=$oPosition->Artikel->cURLFull title=$oPosition->cName|transByISO|escape:'html'}{$oPosition->cName|transByISO}{/link}</p>
                                            {else}
                                            <p>{$oPosition->cName|transByISO}</p>
                                            {/if}
                                        {/block}
                                        {block name='checkout-inc-order-items-product-data'}
                                            ...
                                            <ul class="list-unstyled text-muted-util small">
                                                ...
                                                && isset($oPosition->Artikel->dMHD)
                                                && isset($oPosition->Artikel->dMHD_de)
                                ...
                                {block name='checkout-inc-order-items-price-single'}
                                    {if $Einstellungen.kaufabwicklung.bestellvorgang_einzelpreise_anzeigen === 'Y'}
                                        {col cols=$cols lg=3 class="checkout-items-item-price-single text-nowrap-util text-lg-center"}
                                            {if $oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_ARTIKEL}
                                                {if (!$oPosition->istKonfigVater() || !isset($oPosition->oKonfig_arr) || $oPosition->oKonfig_arr|count === 0) || $oPosition->cResponsibility === 'admorris_pro_extra_product'}
                                                    <span class="checkout-items-item-title d-lg-none">{lang key="pricePerUnit" section="productDetails"}:</span>{$oPosition->cEinzelpreisLocalized[$NettoPreise][$smarty.session.cWaehrungName]}
                                                {/if}
                                            {/if}
                                        {/col}
                                    {/if}
                                ...
                                {block name='checkout-inc-order-items-quantity'}
                                    {col cols=$cols lg=2 class="checkout-items-item-quantity text-nowrap-util text-lg-center"}
                                        <span class="checkout-items-item-title d-lg-none">{lang key="quantity" section="checkout"}:</span> {$oPosition->nAnzahl|replace_delim} {if !empty($oPosition->Artikel->cEinheit)}{$oPosition->Artikel->cEinheit}{/if}
                                    {/col}
                                {/block}
                            {/block}
            ...
            {col xl=5 md=6 class='checkout-items-total'}
                ...
                {block name='checkout-inc-order-items-shipping'}
                    {if isset($oSpezialseiten_arr[$smarty.const.LINKTYP_VERSAND])}
                        {* tplscope !== confirmation is needed because $FavourableShipping is not NULL anymore *}
                        {if isset($FavourableShipping) && $tplscope !== 'confirmation'}
                            {if $NettoPreise}
