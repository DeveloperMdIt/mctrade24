{block name='checkout-inc-steps'}
    ...
    {if $bestellschritt[1] != 3}
        {nav class='stepper checkout-steps flex-nowrap' tag='nav' aria=["label"=>"{lang key='secureCheckout' section='checkout'}"]}
            {col class="nav-item step step-active {if $step1_active}step-current{else}col-auto{/if}"}
                {block name='checkout-inc-steps-first'}
                    {link href="{get_static_route id='bestellvorgang.php'}?editRechnungsadresse=1"
                        ...
                        <div class="step-content">
                            ...
                            {/badge}
                            <span class="step-text {if !$step1_active}d-none d-inline-block--stepper{/if}">
                                {lang section='account data' key='billingAndDeliveryAddress'}
                            </span>
                            {if $step2_active || $step3_active}
                                {$admIcon->renderIcon('check', 'icon-content icon-content--center step-check')}
                            {/if}
                        </div>
                    {/link}
                {/block}
            {/col}
            {col class="nav-item step {if $step2_active || $step3_active}step-active{/if} {if $step2_active}step-current{else}col-auto{/if}"}
                {block name='checkout-inc-steps-second'}
                    {link href="{get_static_route id='bestellvorgang.php'}?editVersandart=1"
                        ...
                        <div class="step-content">
                            ...
                            {/badge}
                            <span class="step-text {if !$step2_active}d-none d-inline-block--stepper{/if}">
                ...
                {/block}
            {/col}
            {col class="nav-item step {if $step3_active}step-active step-current{else}col-auto{/if}"}
                {block name='checkout-inc-steps-third'}
                    <div class="step-content">
                        {badge variant="{if $step3_active}primary{else}secondary{/if}" class="badge-pill"}
                            <span class="badge-count">3</span>
                        {/badge}
                        <span class="step-text {if !$step3_active}d-none d-inline-block--stepper{/if}">
